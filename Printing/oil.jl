...
results_DP = zeros(8,50,6)
results_DP_old = zeros(8,50,6)
results_RJT = zeros(8,50,6)
for i in 1:8
    results_DP[i,:,:] = readdlm("../Results/oil/results_DP_"*string(i)*".csv", ',')
end

for i in 1:8
    if isfile("../Results/oil/RJT_"*string(i)*".csv")
        results_RJT[i,:,:] = readdlm("../Results/oil/RJT_"*string(i)*".csv", ',')
    end
end

for i in 1:8
    if isfile("../Results/oil/DP_old_"*string(i)*".csv")
        results_DP_old[i,:,:] = readdlm("../Results/oil/DP_old_"*string(i)*".csv", ',')
    end
end

mean_total_DP = zeros(8)
mean_total_DP_old = zeros(8)
mean_total_RJT = zeros(8)
mean_sol_time_DP = zeros(8)
mean_sol_time_DP_old = zeros(8)
mean_sol_time_RJT = zeros(8)
no_optimal_RJT = zeros(8)
no_feasible_RJT = zeros(8)
no_optimal_DP_old = zeros(8)
no_feasible_DP_old = zeros(8)
for i in 1:8
...
blah = [Int(i) for i in 1:8]

column_labels = ["M", "M. Tot DP", "M. Sol DP", "M. Tot DP old", "M. Sol DP old", "No Opt DP old", "No Feas DP old", "M. Tot RJT", "M. Sol RJT", "No Opt RJT", "No Feas RJT"]
