...
for i in 1:9
    results_DP[i,:,:] = readdlm("../Results/turbine/DP_NP_"*string(i+2)*".csv", ',')
end
for i in 1:9
    if isfile("../Results/turbine/DP_old_"*string(i+2)*".csv")
        results_DP_old[i,:,:] = readdlm("../Results/turbine/DP_old_"*string(i+2)*".csv", ',')
    end
end

for i in 1:9
    if isfile("../Results/turbine/RJT_"*string(i+2)*".csv")
        results_RJT[i,:,:] = readdlm("../Results/turbine/RJT_"*string(i+2)*".csv", ',')
    end
end
