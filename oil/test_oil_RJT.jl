...
function RJT_model(diagram::InfluenceDiagram)
    ...
    dt4 = Dates.value(round(now() - time,Second))
    if dt4 >= 3600
        return (3700.0, dt4, 3700.0, 3700.0, 0.0, "TIMEOUT")
    end
    @info("Starting the optimization process.")
    @info("Time: ", now())
    optimizer = optimizer_with_attributes(Gurobi.Optimizer,"OptimalityTol" => 1e-09, "MIPGap" => 0)
    set_optimizer(model3, optimizer)
    dt7 = Dates.value(round(now() - time,Second)) - dt4 
    if dt4 + dt7 >= 3600
        return (3700.0, dt4, dt7, 3700.0, 0.0, "TIMEOUT")
    end
    set_time_limit_sec(model3, 3600.0-dt7-dt4)
    optimize!(model3)
    dt9 = Dates.value(round(now() - time,Second))
...
no_rounds = 50
    ...
    for p in 1:no_rounds
        ...
        scaler = abs(minimum(U[:,:]) + minimum(V[:,:]) - 1)
...
writedlm("../Results/oil/RJT_$No_tests.csv", results_RJT, ',')
end
