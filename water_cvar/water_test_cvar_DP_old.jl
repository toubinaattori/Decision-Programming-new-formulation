...
for No_states in 2:10
    ...
    for p in 1:50
        ...
        println("old_DP model building time with jl-package: ",solutions[2])
        println("old_DP solve time with jl-package: ", solutions[3])
        println("old_DP total: ", solutions[1])
        println("old_DP other time: ", solutions[5])
        println("old_DP objective value: ", solutions[4])
        println("iteration: ", p)
    ...
    writedlm("../Results/water_cvar/results_DP_old_$No_states.csv", results_old_DP, ',')
